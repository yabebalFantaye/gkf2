...
$ are Gaussian and isotropic with zero mean %From the addition theorem for spherical harmonics,
$ 
	\mathbb{E}[f_{\ell }(x)] =0 $ and variance $ }\mathbb{E}[f_{\ell }(x)^{2}]=1.$ The covariance function is given by $$
...
In this paper we aim at the implementation of Minkowski functionals/Lipschitz-Killing curvatures (to be defined below) on the spherical harmonic/Fourier components of observed data. Let us first recall the well-known spectral representation theorem for spherical random fields, which states that the following identity holds, in the $L^2$ sense:
\begin{equation}
...
\caption{Analytical vs simulation variance comparison. The red
  ...
  simulation variances, while the solid blue curves (overlapping with
  the dash-dash red curves) are the percentage
...
\begin{figure} %[H]
\begin{center}
  \includegraphics[width=0.32\textwidth,angle=0]{\figdir{criticals_nops_Critical.pdf}}
  \includegraphics[width=0.32\textwidth,angle=0]{\figdir{criticals_nops_Extrema.pdf}}
  \includegraphics[width=0.32\textwidth,angle=0]{\figdir{criticals_nops_Saddle.pdf}}  
...
\begin{figure} %[H]
\begin{center}
  \includegraphics[width=1\textwidth,angle=0]{\figdir{var_nops_criticals_extrema_Saddle.pdf}}  
...
\begin{figure} %[H]
\begin{center}
  \includegraphics[width=1\textwidth,angle=0]{\figdir{ap3_b100nj10_u5_3sig_ps0_mfu_critu_corrcoef.png}}  
\caption{Correlation between and among the three Minkowski
  Functionals and Critical points at different threshold values (as
  shown in the axis) and different multipole values (left and right
  panels). The strong positive and negative correlation at $u \ne 0$
  is such that knowing a single value
  determines the rest of the structure. \label{fig:var_crit}}
\end{center}
\end{figure}
